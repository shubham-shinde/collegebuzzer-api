            ...
            if (index + 1 === files.length) {
              ...
              return res.json({
                ...
                show_msg: {
                  h: "Post Saved",
                  detail:
                    "Your post has been saved. Kindly refresh the window to see the post.",
                },
                status: true,
                code: 204,
              });
        ...
        fs.readFile(element.desination + NameOfPic, (err, pic) => {
          ...
          s3Bucket.putObject(data, function(err, data) {
            if (err) {
              ...
              fs.unlink(element.desination + NameOfPic, () =>
                console.log("deleted")
              );
              im.resize(
                {
                  ...
                  fs.readFile(element.desination + NameOfPicPre, (err, pic) => {
                    ...
                    s3Bucket.putObject(data, function(err, data) {
                      if (err) {
                        ...
                        console.log("succesfully uploaded the image!");
                        fs.unlink(element.desination + NameOfPicPre, () =>
                          console.log("deleted")
                        );
                        fs.unlink(element.path, () => console.log("deleted"));
                        if (index + 1 === files.length) {
  ...
  if (req.session.role === ROLES[0]) {
    Students.findOne({ _id: req.session.Id }).exec((err, student) => {
      ...
      const new_post = STU_POSTS({
        ...
        n_videos: req.files.reduce(
          (sum, video) => (video.mimetype === "video/mp4" ? sum + 1 : sum),
          0
        ),
        p_pic: student.p_pic,
        likes: [],
      ...
      new_post.save((err, post) => {
        ...
        student.save((err) => {
          ...
          if (req.files.length >= 1) {
            const filess = [
              ...req.files.filter((item) => item.mimetype !== "video/mp4"),
              ...req.files.filter((item) => item.mimetype === "video/mp4"),
            ];
